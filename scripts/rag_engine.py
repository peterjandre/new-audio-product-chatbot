...
2. Generates responses using Google Gemini
...
class RAGEngine:
    ...
    Supports both OpenAI and Google Gemini for generation.
    """
    ...
    def __init__(
        ...
        generation_provider: str = "openai",  # "openai" or "gemini"
        ...
        gemini_api_key:
            Google Gemini API key (default: from GEMINI_API_KEY env var)
        openai_api_key:
            OpenAI API key (default: from OPENAI_API_KEY env var)
        embedding_model:
            OpenAI embedding model to use
        generation_provider:
            Which provider to use for generation: "openai" or "gemini" (default: "openai")
        openai_model:
            OpenAI model to use for generation (default: "gpt-3.5-turbo")
        gemini_model:
            Gemini model to use for generation (default: "gemini-1.5-flash")
        """
        # Load FAISS index
        self.faiss_index = FAISSIndex(index_path, metadata_path)
        
        # Store generation provider
        self.generation_provider = generation_provider.lower()
        if self.generation_provider not in ("openai", "gemini"):
            raise ValueError(f"generation_provider must be 'openai' or 'gemini', got '{generation_provider}'")
        ...
        # Configure OpenAI for embeddings (always needed)
        if OpenAI is None:
            raise RuntimeError(
        ...
        print(f"  - Generation provider: {self.generation_provider}")
    ...
    def _build_prompt(self, query: str, context: str) -> str:
        """
        Build the prompt for Gemini with query and retrieved context.
...
Answer:"""
    ...
    def query(
        ...
        temperature:
            Temperature for Gemini generation (default: 0.3)
        max_retries:
        ...
        # Step 5: Generate answer with selected provider
        attempt = 0
        while attempt < max_retries:
            ...
            try:
                ...
                if not answer:
                    raise RuntimeError(f"Empty response from {self.generation_provider}")
...
def main():
    ...
    parser.add_argument(
        ...
        default="openai",
        help="Generation provider: 'openai' or 'gemini' (default: openai)",
    )
    parser.add_argument(
        "--openai-model",
